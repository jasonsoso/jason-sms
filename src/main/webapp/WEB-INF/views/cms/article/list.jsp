...
<body>
		...
		<div id="container">
			...
			<div id="main-content">
			<form id="myForm" action="${ctx}/cms/article/list/" method="get">
									  ...
									  <c:choose>
									  	<c:when test="${not empty page.result}">
										  	...
										  	<tr class="even">
											    <td class="checktd">
											    	<div class="checker">
											    	<span><input type="checkbox" name="items" id="checkbox" value="${article.id}" class="group-checkable" /></span>
											    	</div>
											    </td>
											    <td>${article.cover}</td>
												<td>${article.title}</td>
											    ...
											    <td>
													<a href="${ctx}/cms/article/${article.id}/edit/"  class="do-btn edit-btn"><span>编辑</span></a>
											    </td>
										  	</tr>
										  	</c:forEach>
									  	</c:when>
									  	<c:otherwise>
											<tr><td colspan="12" align="center"><b>暂无内容</b></td></tr>
									  	</c:otherwise>
									  </c:choose>
	...
	$(function(){
		...
		$("#del").click(function(){
			...
			if(confirm("你确定要删除这些内容吗?")){
				...
				$("#myForm").attr("action", "${ctx}/cms/article/delete/")
							.attr("method","post")
		...
		$("#add").click(function(){
			location.href = "${ctx}/cms/article/create/";
		});
	});
</script>
