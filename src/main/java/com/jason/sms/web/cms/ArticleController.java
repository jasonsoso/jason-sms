...
public class ArticleController extends ControllerSupport {
	...
	private static final String REDIRECT_LIST = "redirect:/cms/article/list/";
	private static final String FORM = "cms/article/form";
	private static final String LIST = "cms/article/list";
	...
	public String create(Model model) {
		model.addAttribute(new Article())
			.addAttribute("categories", categoryService.queryCategories());
		return FORM;
	}

	@RequestMapping(value = "/create/", method = POST)
	public String create(@Valid Article entity, BindingResult result,RedirectAttributes redirectAttributes) {
		if (result.hasErrors()) {
			error(redirectAttributes,"创建文章失败，请核对数据后重试");
			return REDIRECT_LIST;
		}
		articleService.save(entity);
		success(redirectAttributes,"文章创建成功");
		return REDIRECT_LIST;
	}
	...
	public String edit(@PathVariable("id") Long id, HttpServletRequest request,RedirectAttributes redirectAttributes) {
		try {
			...
			error(redirectAttributes,"文章修改失败，请核对数据重试",e);
		}
		return REDIRECT_LIST;
	}
